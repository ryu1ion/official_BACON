...
try:
    from transformers.cache_utils import Cache, DynamicCache, StaticCache, QuantizedCacheConfig
except ImportError:
...
from bacon.bacon_utils import  init_snapkv, init_pyramidkv, init_adakv, init_sparsemm, init_mask,init_mixsparsemm
import math
try:
    ...
    def _missing_flash_attn(*args, **kwargs):
        raise RuntimeError("flash-attn is required for Qwen2-VL compressed attention paths.")

    index_first_axis = pad_input = unpad_input = _missing_flash_attn
from bacon.bacon_utils import DynamicCacheSplitHeadFlatten
...
def qwen_flash_attn2_forward_SparseMM(
    ...
    return attn_output, attn_weights, past_key_value
def qwen_flash_attn2_forward_MixSparseMM(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
...
def adakv_qwen_forward(
    ...
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    past_key_values: Optional[List[torch.FloatTensor]] = None, # [adakv]
