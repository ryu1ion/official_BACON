...
torch::Tensor update_flatten_view(torch::Tensor &cache, torch::Tensor &state, torch::Tensor &headlens, torch::Tensor& cu_headlens) {
  ...
  TORCH_CHECK(headlens.is_cuda(), "headlens must be a CUDA tensor");
  TORCH_CHECK(cu_headlens.is_cuda(), "cu_headlens must be a CUDA tensor");
  TORCH_CHECK(cache.device() == state.device(), "cache and state must be on the same device");
  TORCH_CHECK(cache.device() == headlens.device(), "cache and headlens must be on the same device");
  TORCH_CHECK(cache.device() == cu_headlens.device(), "cache and cu_headlens must be on the same device");
  TORCH_CHECK(cache.is_contiguous(), "cache must be contiguous");
  TORCH_CHECK(state.is_contiguous(), "state must be contiguous");
  TORCH_CHECK(headlens.is_contiguous(), "headlens must be contiguous");
  TORCH_CHECK(cu_headlens.is_contiguous(), "cu_headlens must be contiguous");
  ...
  const c10::cuda::CUDAGuard device_guard(cache.device());
  ...
  FP16_SWITCH(cache.dtype() == torch::kFloat16, [&] {
      ...
      kernel<<<grid, block, 0, at::cuda::getCurrentCUDAStream()>>>((elem_type*)out.data_ptr(), (elem_type*)cache.data_ptr(), (elem_type*)state.data_ptr(), (int*)headlens.data_ptr(), (int*)cu_headlens.data_ptr(), head_dim);
