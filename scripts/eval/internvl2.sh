#!/usr/bin/env bash
# Evaluate InternVL3-8B on a multimodal benchmark with a chosen KV-compression
# backbone (snapkv / pyramidkv / adakv) and a chosen within-head score
# (`base`, `mixkv`, or `bacon`).
#
# Override any knob via env vars, e.g.
#   BUDGET=128 TASK=docvqa SELECT=bacon bash scripts/eval/internvl2.sh
set -euo pipefail
...
INTERNVL_MODEL="${INTERNVL_MODEL:-OpenGVLab/InternVL3-8B}"
OUTPUT_DIR="${OUTPUT_DIR:-./logs}"
RESULT_DIR="${RESULT_DIR:-./eval_results/internvl3_results}"
...
echo "==> InternVL3  task=${TASK}  backbone=${METHOD}  select=${SELECT}  budget=${BUDGET}"
...
python3 -m accelerate.commands.launch \
    ...
    -m lmms_eval \
        --model internvl2 \
        --model_args "pretrained=${INTERNVL_MODEL}" \
        --tasks "$TASK" \
        --batch_size 1 \
        --log_samples \
